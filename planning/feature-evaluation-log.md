...
- **Decision**:
  - Ship an **"inMemory" adapter driver plugin as a test-support fixture** (following the ref's `flysystem_test_adapter` pattern: a test-only driver so kernel tests can configure a full scheme backed by a real adapter — exercising the wrapper, decorated manager, and registration/resolution path end-to-end against real adapter code, without mocks, network, or disk).
